...
class MarketplaceSmartContract(ARC4Contract):
    assetId: UInt64
    unitaryPrice: UInt64
    ...
    def deleteApplication(self) -> None:
        ...
        # Close out the asset transfer to the creator
        itxn.AssetTransfer(
            xfer_asset=self.assetId,  # Explicitly casting self.assetId to UInt64
            asset_receiver=Global.creator_address,  # Receiver should be the creator
            asset_amount=0,
            asset_close_to=Global.creator_address,
            # Close asset to creator
        ).submit()

        # Close out any remaining balance to the creator
        itxn.Payment(
